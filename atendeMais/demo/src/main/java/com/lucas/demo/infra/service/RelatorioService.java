...
import com.lucas.demo.domain.exceptions.ErroRelatorioException;
...
public class RelatorioService {
	...
	private ApagarPedidos apagarPedidos;

	public RelatorioService(PedidoService pedidoServico, ApagarPedidos apagarPedidos){
		this.pedidoServico = pedidoServico;
		this.apagarPedidos = apagarPedidos;
	}
	...
	public void criarTodosRelatoriosDiarios(LocalDate data, String estabelecimentoId) {
		...
		for (int i = 0; i < diasSemanas.size(); i++) {
			...
			gerarPdf(dia, estabelecimentoId);
		}
	}
	...
	public void verificarPossivelPdfSemanalECriar(LocalDate data, CaminhoInfo caminhoInfo, String destino,
		...
		if (data.getDayOfWeek() == DayOfWeek.SUNDAY) {
			...
			this.gerarPDFSemanal(destino, destinoSemanal, data, estabelecimentoId, semanaDoMes, nomeMes);

			this.criarTodosRelatoriosDiarios(data, estabelecimentoId);

			apagarPedidos.verificarEApagarPedidosSemana(obterDiasDaSemanaNull(data),
					MudancaSO.obterCaminhoPedidos(estabelecimentoId));
		}
	}

	public File gerarPdf(LocalDate data, String estabelecimentoId) {
		String destino = caminhoRelatorio(data, estabelecimentoId);
		File relatorioDiario = new File(destino);

		CaminhoInfo caminhoInfo = MudancaSO.obterCaminhoParaRelatorios(estabelecimentoId);

		this.validarDiretorio(caminhoInfo.getDiretorio());
		PedidosContext pedidoContext = this.lerPedidosArquivo(data, estabelecimentoId);

		this.validarPdfDiariosExist(relatorioDiario, pedidoContext, destino, data);

		this.verificarPossivelPdfSemanalECriar(data, caminhoInfo, destino, estabelecimentoId);
	...
	public void gerarPDFSemanal(String destino, String destinoSemanal, LocalDate data, String estabelecimentoId,
			int semanaDoMes, String mes) {
		Document document = new Document();
		try {
			List<LocalDate> diasDaSemana = this.obterDiasDaSemanaNull(data);

			List<PedidosContext> relatoriosDiarios = this.lerOsRelatoriosDiarios(diasDaSemana, estabelecimentoId);
	...
	public List<PedidosContext> lerOsRelatoriosDiarios(List<LocalDate> diasDaSemana, String estabelecimentoId) {
		List<PedidosContext> relatoriosDiarios = new ArrayList<>();

		for (LocalDate dia : diasDaSemana) {
			if (dia != null) {
				try {
					PedidosContext pedidosContext = this.lerPedidosArquivo(dia, estabelecimentoId);
	...
	protected String caminhoRelatorio(LocalDate data, String estabelecimentoId) {
		CaminhoInfo caminhoInfo = MudancaSO.obterCaminhoParaRelatorios(estabelecimentoId);
		String caminhoArq = caminhoInfo.getCaminhoArquivo();
		return caminhoArq + data + ".pdf";
	}

	protected String caminhoArquivoPedido(LocalDate data, String estabelecimentoId) {
		CaminhoInfo caminhoInfo = MudancaSO.obterCaminhoPedidos(estabelecimentoId);
		String caminhoArq = caminhoInfo.getCaminhoArquivo();
		return caminhoArq + data + ".json";
	}

	public PedidosContext lerPedidosArquivo(LocalDate data, String estabelecimentoId) {
		String caminhoArq = this.caminhoArquivoPedido(data, estabelecimentoId);
